...
def training_loop(params, device, slurm_localid, gpus_per_node):
    ...
    if dist.is_initialized() and gpus_per_node > 0:
        model = DistributedDataParallel( # Wrap model with DDP.
                model, 
    ...
    total_weights = rng.random((params['num_epochs'], 5))
    pressure_weights = total_weights[:, [0, 1, 2, 3, 3]] / total_weights[:, [0, 1, 2, 3, 3]].sum(1, keepdims=1) * (3.00 + 0.6 + 1.5 + 0.77 + 0.54)
    surface_weights = total_weights[:, [4, 3, 3, 2]] / total_weights[:, [4, 3, 3, 2]].sum(1, keepdims=1) * (1.5 + 0.77 + 0.66 + 3.0)
    #
    #pressure_weights = torch.tensor(pressure_weights).to(torch.float32)
    #surface_weights  = torch.tensor(surface_weights).to(torch.float32)
    ...
    for epoch in range(start_epoch, start_epoch + num_epochs):
        ...
        for i, data in enumerate(train_data_loader):        # Load weather data at time t as the input; load weather data at time t+1/3/6/24 as the output
            ...
            with torch.autocast(device_type="cuda"):
                ...
                pressure_weights_epoch = torch.tensor(pressure_weights[epoch]).to(torch.float32).view(1, 5, 1, 1, 1).to(device)
                surface_weights_epoch  = torch.tensor(surface_weights[epoch]).to(torch.float32).view(1, 4, 1, 1).to(device)
...
if __name__ == '__main__':
    ...
    params['hash'] = None# "20240426_616578378"
    params['Lite'] = True
    params['daily'] = False
    params['save_counter'] = None # 61 #  100
    params['model_name'] = 'random_weights/'
