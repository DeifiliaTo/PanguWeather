...
class PanguModel(nn.Module):
  def __init__(self, C=int(192*1.5), patch_size=(8, 8), device='cpu'):
    ...
    # Four basic layers
    self.layer1 = EarthSpecificLayer2D(2, self.C, drop_list[:2], 6,  input_shape=[8, 93], device=device, input_resolution=(93, 180), window_size=torch.tensor([6, 12]))
    self.layer2 = EarthSpecificLayer2D(6, 2*self.C, drop_list[2:], 12, input_shape=[8, 46], device=device, input_resolution=(46, 90), window_size=torch.tensor([6, 12]))
    self.layer3 = EarthSpecificLayer2D(6, 2*self.C, drop_list[2:], 12, input_shape=[8, 46], device=device, input_resolution=(46, 90), window_size=torch.tensor([6, 12]))
    self.layer4 = EarthSpecificLayer2D(2, self.C, drop_list[:2], 6,  input_shape=[8, 93], device=device, input_resolution=(93, 180), window_size=torch.tensor([6, 12]))
