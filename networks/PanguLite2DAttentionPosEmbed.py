...
class PanguModel(nn.Module):
  def __init__(self, C=int(192*1.5), patch_size=(8, 8), device='cpu'):
    super().__init__()
    ...
    self.absolute_pos_embed = nn.Parameter(torch.zeros(1, 91, self.C))
    trunc_normal_(self.absolute_pos_embed, std=0.02)

    # Four basic layers
    self.layer1 = EarthSpecificLayer2DNoBias(2, self.C, drop_list[:2], 6,  input_shape=[8, 93], device=device, input_resolution=(93, 180), window_size=torch.tensor([6, 12]))
    self.layer2 = EarthSpecificLayer2DNoBias(6, 2*self.C, drop_list[2:], 12, input_shape=[8, 46], device=device, input_resolution=(46, 90), window_size=torch.tensor([6, 12]))
    self.layer3 = EarthSpecificLayer2DNoBias(6, 2*self.C, drop_list[2:], 12, input_shape=[8, 46], device=device, input_resolution=(46, 90), window_size=torch.tensor([6, 12]))
    self.layer4 = EarthSpecificLayer2DNoBias(2, self.C, drop_list[:2], 6,  input_shape=[8, 93], device=device, input_resolution=(93, 180), window_size=torch.tensor([6, 12]))

    # Upsample and downsample
    self.upsample = UpSample2D(self.C*2, self.C, nLat=46, nLon=90, lat_crop=(0, 1), lon_crop=(0, 0))
  ...
  def forward(self, input, input_surface):
    ...
    x = x.reshape(x.shape[0], 91, 180, self.C) + self.absolute_pos_embed.unsqueeze(2)
