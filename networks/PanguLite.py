...
class PanguModel(nn.Module):
  """Class definition of Pangu-Lite model."""

  def __init__(self, dim=192, patch_size=(2, 8, 8), device='cpu'):
    """
    Initialize Pangu-Lite model.
    ...
    self.layer3 = EarthSpecificLayerAbsolute(6, 2*self.dim, drop_list[2:], 12, input_shape=[8, 46], device=device, input_resolution=(8, 46, 90), window_size=torch.tensor([2, 6, 12]))
    self.layer4 = EarthSpecificLayerAbsolute(2, self.dim, drop_list[:2], 6,  input_shape=[8, 93], device=device, input_resolution=(8, 93, 180), window_size=torch.tensor([2, 6, 12]))
  ...
  def forward(self, input, input_surface):
    """
    Forward pass of Pangu-Lite model.
