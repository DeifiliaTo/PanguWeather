...
def training_loop(params, device, slurm_localid, gpus_per_node):
    print("model dir", params['save_dir'][params['model']])
    ...
    for epoch in range(params['num_epochs']):
        ...
        with torch.no_grad():
            ...
            val_loss, MSE, acc, total_samples, dt_validation = eval.get_loss(model, valid_data_loader, device, loss1, loss2, lat_crop=params['lat_crop'], lon_crop=params['lon_crop'])

            if rank == 0:
                valid_loss_history.append(val_loss.item())
                if (val_loss < best_val_loss) or (epoch % 20 == 0):
                ...
                print(f'| Mean L1 Validation Loss: {val_loss :.3f} ')
                print(f'| MSE T500: {MSE[0] :.3f} ')
                print(f'| MSE Z500: {MSE[1] :.3f} ')
                print(f'| MSE T2M:  {MSE[2] :.3f} ')
                print(f'| MSE U10:  {MSE[3] :.3f} ')
                print(f'| MSE V10:  {MSE[4] :.3f} ')
                print(f'| ACC T500: {acc[0] :.3f} ')
                print(f'| ACC Z500: {acc[1] :.3f} ')
                print(f'| ACC T2M:  {acc[2] :.3f} ')
                print(f'| ACC U10:  {acc[3] :.3f} ')
                print(f'| ACC V10:  {acc[4] :.3f} ')
...
if __name__ == '__main__':
    ...
    params['C'] = 192
    params['subset_size'] = None
    params['validation_subset_size'] = None
    params['restart'] = False
    params['hash'] = None # "20240319_" + str(338453693)
    params['Lite'] = True
    params['save_counter'] = None
